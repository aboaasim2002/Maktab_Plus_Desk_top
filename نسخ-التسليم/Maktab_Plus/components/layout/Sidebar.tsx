...
import {
  ...
  Settings, ReceiptText, ShieldCheck, ScrollText, LogOut, KeyRound, ArrowRight,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  async function leaveOrganization() {
    const response = await fetch('/api/organizations/context', { method: 'DELETE' });
    if (!response.ok) {
      const data = await response.json().catch(() => ({ error: 'تعذر العودة إلى قائمة المنشآت' }));
  ...
  return (
    <aside className="fixed right-0 top-0 h-screen w-64 bg-white border-l border-gray-100 shadow-sm flex flex-col z-30 print-hidden">
      ...
      <div className="px-4 py-4 border-t border-gray-100">
        {user?.role === 'platform_owner' && user.is_impersonating && (
          <button
            onClick={leaveOrganization}
            className="mb-2 w-full flex items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800 hover:bg-amber-100"
          >
            <ArrowRight className="w-4 h-4" />
            العودة إلى جميع المنشآت
          </button>
        )}
