...
export function buildInvoicePrintHtml(invoice: InvoicePrintData, settings: InvoicePrintSettings): string {
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          body { direction: rtl; color: #111827; font-family: Arial, Tahoma, sans-serif; font-size: 13px; }
          .invoice-sheet { width: 100%; min-height: 255mm; border: 2px solid #1f2937; padding: 12mm; }
          .header { text-align: center; border-bottom: 2px solid #1f2937; padding-bottom: 12px; margin-bottom: 18px; }
          h1 { margin: 0; font-size: 24px; }
          .address { margin: 5px 0 0; color: #4b5563; font-size: 12px; }
          h2 { margin: 14px 0 0; font-size: 20px; }
          .meta { display: grid; grid-template-columns: 1fr 1fr; gap: 10px 24px; margin-bottom: 18px; }
          .meta-item { border: 1px solid #9ca3af; padding: 9px 12px; }
          ...
          .summary-total { display: flex; justify-content: space-between; gap: 20px; padding: 12px; background: #f3f4f6; font-size: 18px; font-weight: 700; }
          .amount-text { border-top: 1px solid #1f2937; padding: 12px; line-height: 1.8; }
          .footer { display: flex; justify-content: space-between; gap: 20px; margin-top: 40px; border-top: 1px solid #d1d5db; padding-top: 12px; color: #4b5563; font-size: 11px; }
        </style>
      </head>
      <body>
...
export function writeInvoiceToPrintWindow(
  printWindow: Window,
  invoice: InvoicePrintData,
  settings: InvoicePrintSettings,
) {
  printWindow.document.open();
  printWindow.document.write(buildInvoicePrintHtml(invoice, settings));
  printWindow.document.close();
}

export default function InvoicePrint({ invoice }: { invoice: InvoicePrintData }) {
  ...
  function handlePrint() {
    ...
    writeInvoiceToPrintWindow(printWindow, invoice, settings);
  }
