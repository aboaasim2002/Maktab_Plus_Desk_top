...
export async function GET() {
  const isTrial = process.env.APP_IS_TRIAL === '1';
  const expiresAt = process.env.APP_TRIAL_EXPIRES_AT || null;
  const envDaysRemaining = Number(process.env.APP_TRIAL_DAYS_REMAINING);
