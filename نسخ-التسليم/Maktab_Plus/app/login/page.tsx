...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!user) return;
    if (user.role === 'platform_owner') router.replace('/organizations');
    else if (user.role === 'office_owner' || user.permission_mode === 'all' || user.permissions.includes('dashboard.view')) {
  ...
  async function submit(event: FormEvent) {
    ...
    const landingPage =
      user?.role === 'platform_owner' ? '/organizations' :
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-indigo-950 via-indigo-800 to-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={submit} autoComplete="off" className="p-8 space-y-5">
          ...
          <label className="block">
            ...
            <span className="relative block">
              ...
              <input dir="ltr" name="login_username" autoComplete="off" className="input-field pr-10 text-left" value={username} onChange={(e) => setUsername(e.target.value)} autoFocus required />
            </span>
          </label>
          <label className="block">
            ...
            <span className="relative block">
              ...
              <input dir="ltr" name="login_password" type="password" autoComplete="new-password" className="input-field pr-10 text-left" value={password} onChange={(e) => setPassword(e.target.value)} required />
            </span>
          </label>
