...
export default function ContractsPage() {
  ...
  const clientPickerValue = clientListOpen
    ? clientSearch
    ...
    return (
      <div className="m-4 rounded-2xl border-2 border-indigo-200 bg-indigo-50/50 p-5">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="lg:col-span-2">
            <label className="mb-1.5 block text-sm font-semibold text-gray-700">المبلغ بالريال السعودي</label>
                          ...
                          <td className="border border-gray-300 px-3 py-3">
                            {isOperation ? (
                              <div className="flex flex-wrap justify-center gap-2 text-xs">
                                <Link href={`/contracts/${row.operation.id}`} className="text-indigo-700 hover:underline flex items-center gap-1">
                                  <FileText className="w-3.5 h-3.5" /> التفاصيل
                                </Link>
                                {can('operations.edit') && (
                                  <button onClick={() => startEditing(row.operation)} className="text-amber-700 hover:underline flex items-center gap-1">
                                    <Edit2 className="w-3.5 h-3.5" /> تعديل
                                  </button>
                                )}
                                {can('operations.print') && (
                                  <button onClick={() => setDetailOperation(row.operation)} className="text-green-700 hover:underline flex items-center gap-1">
                                    <Printer className="w-3.5 h-3.5" /> طباعة
                                  </button>
                                )}
                                {can('operations.delete') && (
                                  <button onClick={() => deleteMovement(row)} className="text-red-700 hover:underline flex items-center gap-1">
                              ...
                              <div className="flex flex-wrap justify-center gap-2 text-xs">
                                {can('vouchers.edit') && (
                                  <button onClick={() => startEditingVoucher(row.voucher)}
                                    className="text-amber-700 hover:underline flex items-center gap-1">
                                    <Edit2 className="w-3.5 h-3.5" /> تعديل السند
                                  </button>
                                )}
                                {can('vouchers.print') && (
                                  <button onClick={() => setPrintVoucher(row.voucher)}
                                ...
                                {can('vouchers.delete') && (
                                  <button onClick={() => deleteMovement(row)} className="text-red-700 hover:underline flex items-center gap-1">
                                    <Trash2 className="w-3.5 h-3.5" /> حذف السند
                                  </button>
                                )}
                              </div>
                            )}
      ...
      >
        <ClientForm
          ...
          onSuccess={async (savedClient) => {
            ...
            setClients((current) => {
              const withoutSavedClient = current.filter((client) => client.id !== savedClient.id);
              return [...withoutSavedClient, savedClient].sort((a, b) => a.name.localeCompare(b.name, 'ar'));
            });
            selectClient(savedClient);
            setLoadingClients(true);
            const updatedClients = await loadClients();
            if (wasEditing) {
              const updatedClient = updatedClients.find((client) => client.id === savedClient.id);
              if (updatedClient) {
