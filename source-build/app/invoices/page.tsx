...
export default function InvoicesPage() {
  ...
  async function saveInvoice() {
    ...
    setSaving(false);
    if (!response.ok) return setError(data.error || 'تعذر حفظ الفاتورة');
  ...
  async function openInvoice(invoice: Invoice, mode: 'edit' | 'print') {
    ...
    setEditingId(full.id); setInvoiceDate(full.invoice_date); setCustomerName(full.customer_name || '');
              ...
              <div className="card p-6">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">اسم العميل اختياري، وجميع المبالغ بالريال السعودي</p>
                  </div>
                    ...
                    <tbody>
                      {items.map((item, index) => (
                        <tr key={index} className="border-t">
                          ...
                          <td className="p-2 text-center"><button onClick={() => setItems((current) => current.filter((_, i) => i !== index))} className="text-red-500 p-2"><Trash2 className="w-4 h-4" /></button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  <button onClick={() => setItems((current) => [...current, emptyItem()])} className="btn-secondary flex items-center gap-1"><Plus className="w-4 h-4" /> إضافة بند</button>
                ...
                <button onClick={saveInvoice} disabled={saving} className="btn-primary mt-6 px-8 py-3">{saving ? 'جارٍ الحفظ...' : editingId ? 'حفظ التعديلات' : 'حفظ الفاتورة'}</button>
              </div>
            </>
          )}
        </div>
